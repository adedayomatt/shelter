...
**php script.
*/
function follow($agentBusinessName,$agentusername, $follower,$followerid,$followtype){
        GLOBAL $db;
    $get_follow = null;
...
if($agentcookie != null){
 ...
 $getProfile = $db->query_object("SELECT ID,Business_Name,User_ID,token FROM profiles WHERE (token='$agentcookie')");
		 if(is_object($getProfile)){
			...
			$agentProfile['a_followers'] = $data->agent_followers($records['Business_Name']);
			$agentProfile['c_followers'] = $data->client_followers($records['Business_Name']);
			$agentProfile['followings'] = $data->following($records['Business_Name']);
...
if($ctacookie != null){
$getCTA = $d->query_object("SELECT ctaid,name,request,timeCreated,expiryTime FROM cta WHERE (ctaid='$ctacookie')");
if(is_object($getCTA)){
//if CTA is found...
if($getCTA->num_rows != 0){
			$now = time();
...
GLOBAL $data;
			...
			$ctaProfile['followings'] = $data->following($records['name']);
